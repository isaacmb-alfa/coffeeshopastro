import { BaseWPSchema, ProcessPageSchema, BlogsSchemaPosts } from "@/types";
...
export async function pageAPIResponse(endpoint: string, schemaType: 'base'): Promise<BasePageResponse>;
export async function pageAPIResponse(endpoint: string, schemaType: 'process'): Promise<ProcessPageResponse>;
export async function pageAPIResponse(endpoint: string, schemaType: 'blogs'): Promise<BlogResponse>;
export async function pageAPIResponse(
  endpoint: string, 
  schemaType: 'base' | 'process' | 'blogs' = 'base'
): Promise<BasePageResponse | ProcessPageResponse | BlogResponse> {
  try {
    const urlAPI =
    ...
    if (!['base', 'process', 'blogs'].includes(schemaType)) {
      throw new Error(`Invalid schema type: ${schemaType}. Must be 'base', 'process', or 'blogs'`);
    }

    switch (schemaType) {
      case 'base':
      case 'process':
        const data = schemaType === 'base' ? BaseWPSchema.parse(resJSON[0]) : ProcessPageSchema.parse(resJSON[0]);
